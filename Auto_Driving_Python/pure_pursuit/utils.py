...
delta_cmd_raw = np.array([90, 60, 0, -60, -90], np.int16) + 90
...
def pure_pursuit_control(init_state: VehicleState, path, ctrller: car_controllor, flag, timeout):
    '''
    ...
    while ((state.x - path[last_index, 0]) ** 2 + (state.y - path[last_index, 1]) ** 2) ** 0.5 > 5:
        ...
        if flag == False:
            state.delta = state.delta / 4  # 调小角度
            delta_cmd[delta_idx] = (delta_cmd_raw[delta_idx] - 90) / 4 + 90
        else:
            delta_cmd[delta_idx] = delta_cmd_raw[delta_idx]
